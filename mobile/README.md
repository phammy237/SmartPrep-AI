...
**Pantry-ingredient matching is user-reviewable, never silent:** the cooking-flow review screen proposes a pantry match per ingredient (exact `ingredientId` match, same-unit required for an automatic default) and lets the user change it via a picker (`features/recipes/components/PantryItemPickerModal.tsx`) listing their own active, in-stock pantry items - an incompatible-unit or depleted item is shown disabled, never selectable, so the app can never silently pick one. Choosing a different item re-resolves compatibility and re-clamps the deduction to that item's live stock (`utils/nutritionSnapshot.ts#resolvePantryMatch`); a manual remap is marked confidence `'likely'` (vs. `'exact'` for the original catalog-id auto-match) since the user, not an id match, made the call. The user can also explicitly skip an ingredient. Only the final, user-confirmed mapping for each ingredient - whatever `resolvePantryMatch` last resolved to - is sent to `complete_cooking_event`.
