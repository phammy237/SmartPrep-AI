...
**Grocery, kitchen impact, and scan vision/OCR inference are still mock**: those `services/` (and `scanService.processCapture`) read and write an in-memory mock database (`services/mockDb.ts`, seeded from `data/`) through `services/apiSimulation.ts`, which adds artificial latency to mimic real network calls. `hooks/` wraps each service in [TanStack Query](https://tanstack.com/query). Swapping in a real backend means reimplementing the service, following the pattern `userService.ts`/`pantryService.ts`/`recipeService.ts` now use; the hooks and screens above it shouldn't need to change. **Scan _confirmation_ is no longer mock** - see below.
...
**Scan confirmation persists to the real pantry.** `scanService.confirmScan` creates every confirmed detection through the same `create_pantry_item` RPC (via the repository layer) that a manual "Add item" uses - identical validation, identical append-only `added` ledger event, `scan_source = 'scan'`. There is no separate scan insert path and no fallback to mock storage: a rejected row surfaces as a `ScanConfirmError`. What is still mocked: the vision/OCR inference itself (`processCapture` returns canned detections), so scanned items carry `expiration_confidence = 'unknown'` with no fabricated date; and scan _history_ (`getScanHistory`), since there is no `scans` table yet.

**Backlog - Scan confirmation idempotency (not yet implemented).** `create_pantry_item` takes no idempotency key (matching Phase 2's "no client-side idempotency" pantry semantics), so `confirmScan` deliberately reissues every create RPC on retry - a network retry, or a retry after a partial failure, can create duplicate pantry inventory rows and duplicate `added` events. Before production this needs a stable mutation/session identifier (client-generated, reused across retries) threaded from the scan-review session through `confirmScan` into a server-side dedupe (e.g. an idempotency-keyed RPC or a per-detection natural key), so a partial retry converges instead of double-adding. Do not change the Scan implementation for this until it is scheduled or live validation forces it.
